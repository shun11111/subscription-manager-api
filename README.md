...
このプロジェクトでは、AIを活用して効率的に開発を進めます。以下は、AIに最初に指示する際の文言例です。

### 📝 新規画面構築（フロントエンド + バックエンド）

```
{画面名}という管理画面を追加したい。

仕様:
- 一覧画面、詳細画面、新規作成/編集フォーム画面を含む
- 認証が必要
- データ項目: {項目1}, {項目2}, ...

まず、docs/specs/{画面名}.md の仕様書を確認または作成し、
その後 docs/INTEGRATED_DEVELOPMENT_FLOW.md に従って
フロントエンド（subscription-manager-admin）とバックエンド（このリポジトリ）の両方を実装してください。
```

### 🔌 APIのみの修正

```
{画面名}のAPIエンドポイントを修正したい。

修正内容:
- {エンドポイント名}: {具体的な変更点1}
- {エンドポイント名}: {具体的な変更点2}

まず、docs/specs/{画面名}.md の仕様書を更新し、
その後 AI_INSTRUCTIONS.md に従って実装してください。
手順:
1. docs/specs/{画面名}.md を更新
2. make gen-openapi でOpenAPIを更新
3. make gen-code でGoコードを更新
4. 各レイヤ（domain, usecase, infrastructure, handler）を実装
```

### 🔧 フロントエンドとバックエンドの両方を修正

```
{画面名}画面の{具体的な修正内容}を修正したい。
...
まず、docs/specs/{画面名}.md の仕様書を更新し、
その後 docs/INTEGRATED_DEVELOPMENT_FLOW.md に従って
フロントエンド（subscription-manager-admin）とバックエンド（このリポジトリ）の両方を修正してください。
```

### 💡 その他の例

```
- {画面名}のAPIエンドポイントに{機能名}機能を追加したい
- {画面名}のAPIのバリデーションを強化したい
- {画面名}のAPIのエラーハンドリングを改善したい
- {画面名}のAPIのレスポンス形式を変更したい
- {画面名}のAPIに新しいエンドポイントを追加したい
```

**重要:** API実装時は、必ず `AI_INSTRUCTIONS.md` を参照し、Clean Architectureの4層構造（domain, usecase, infrastructure, interface）に従って実装してください。
